...
import 'tasks_screen_winter.dart';
import '../services/api_service.dart';

class NewTaskScreen1 extends StatefulWidget {
  final String username;
  final Task? task;
  const NewTaskScreen1({super.key, required this.username, this.task});

  @override
  State<NewTaskScreen1> createState() => _NewTaskScreen1State();
}

class _NewTaskScreen1State extends State<NewTaskScreen1> {
  ...
  String _characterImage = 'assets/images/toddler.png';
  ...
  Future<void> _fetchUserProgress() async {
    ...
    if (user != null) {
      ...
      if (progress != null) {
        setState(() {
          if (progress.tasksDone >= 12) {
            _characterImage = 'assets/images/adult1.png';
          } else if (progress.tasksDone >= 5) {
            _characterImage = 'assets/images/teen1.png';
          }
        });
      }
    }
    ...
    try {
      ...
      if (ok) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => TasksScreen1(username: widget.username),
          ),
        );
      } else {
        if (!mounted) return;
                        ...
                        children: [
                          ...
                          Container(
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: NotivaColors.panelBorder, width: 2),
                            ),
                            padding: const EdgeInsets.symmetric(horizontal: 12),
                            child: TextFormField(
                              controller: _title,
                              style: const TextStyle(color: NotivaColors.bgTop, fontSize: 18),
                              decoration: const InputDecoration(
                                hintText: 'Title',
                                hintStyle: TextStyle(color: NotivaColors.bgBottom),
                          ...
                          Container(
                            ...
                            child: TextFormField(
                              controller: _notes,
                              style: const TextStyle(color: NotivaColors.bgTop, fontSize: 16),
                              decoration: const InputDecoration(
                                hintText: 'Notes',
                                hintStyle: TextStyle(color: NotivaColors.bgBottom),
                                border: InputBorder.none,
