...
class ProgressScreen extends StatefulWidget {
  ...
  const ProgressScreen({super.key, required this.username});
...
class _ProgressScreenState extends State<ProgressScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _tasks = tasks
            .where((t) => !t.isCompleted && !Task.isInTempCache(t.id!))
            .toList();
  ...
  Future<void> _claimReward() async {
    ...
    final originalCompletedTasks = List<Map<String, dynamic>>.from(
      _userProgress!.completedTasks,
    );
    ...
    try {
      ...
      final newHistoryItems = tempTasks
          .map(
            (task) => {
              'title': task.title,
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Reward claimed and progress saved!')),
          );
          _calculateLevelStats(_userProgress!.tasksDone);
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Scaffold(
        body: Center(
          child: CircularProgressIndicator(color: NotivaColors.gold),
        ),
      );
    }

    final progressForCurrentLevel =
        _tasksDoneInCurrentLevel + Task.getTempCache().length;
  ...
  Widget _buildHeader() {
    return Row(
      ...
      children: [
        TextButton.icon(
          style: TextButton.styleFrom(foregroundColor: NotivaColors.gold),
              ...
              children: [
                ...
                Expanded(
                  child: LinearProgressIndicator(
                    value: _tasksRequiredForLevel == 0
                        ? 0
                        : currentProgress / _tasksRequiredForLevel,
                    backgroundColor: Colors.grey[700],
                    valueColor: AlwaysStoppedAnimation<Color>(
                      isLevelComplete ? Colors.greenAccent : NotivaColors.gold,
                    ),
                    minHeight: 10,
                  ),
                ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(color: NotivaColors.gold, thickness: 1, height: 20),
              Expanded(
                      ...
                      children: [
                        Checkbox(
                          ...
                          onChanged: (bool? value) =>
                              _onTaskCheckedChanged(task, value),
